...
Feature.flowEntries = function(flowData, outPort)
    table.insert(flowData.groups, string.format("group_id=1, type=select, bucket=mod_nw_src=%s,actions=output:%s, bucket=mod_nw_dst=%s, actions=output:%s", conf.new_SRC_IP4, outPort, conf.new_DST_IP4, outPort))
